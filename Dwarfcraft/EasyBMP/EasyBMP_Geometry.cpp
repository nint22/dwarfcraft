 ...
 */
 ...
 for (CurrentPixel = 0; CurrentPixel <= NumberOfPixels; CurrentPixel++ )
 {
     ...
     // This hack was written by Jeremy bridon May 3rd, as a way
     // to prevent the bad-behavior of out-of-bound lines. The trick
     // is to always attempt to draw, even if the line is out of the
     // screen, we only do pixel-inclusion checking
     
     // Only draw if within bounds
     if(x >= 0 && y >= 0 && x < Image.TellWidth() && y < Image.TellHeight())
     {
